...
function renderAllImages() {
  drawingContainer.querySelectorAll('.annotation-image').forEach(e => e.remove());
  images.forEach(r => renderImage(r));
}

function renderImage(imgData) {
    const wrap = createEl('div', {class: 'annotation-image','data-id': imgData.id});
    applyImageStyle(wrap, imgData);
    const imgEl = createEl('img', { src: imgData.src, draggable: 'false', alt: '' });
    ...
    eventManager.on(pinBtn, 'click', (e) => {
        ...
        applyImageStyle(wrap, imgData);
        pinBtn.innerHTML = imgData.fixed ? '<svg t="1759978122492" class="icon" viewBox="0 0 1024 1024" version="1.1" xmlns="http://www.w3.org/2000/svg" p-id="1768" width="200" height="200"><path d="M697.464355 375.861272l-20.716763-208.154143h73.988439q16.770713 0 29.102119-12.331406T792.169557 126.273603V41.433526q0-16.770713-12.331407-29.102119T750.736031 0H273.263969q-16.770713 0-29.102119 12.331407T231.830443 41.433526v84.840077q0 16.770713 12.331407 29.10212T273.263969 167.707129h73.988439l-20.716763 208.154143q-68.069364 30.581888-107.529865 80.894027-43.406551 54.258189-43.406551 118.381503 0 17.757225 12.331407 30.088632T217.032755 617.55684h238.736031v181.518305q0 3.94605 1.973025 6.905587l41.433526 83.853565q3.94605 7.8921 12.824663 7.8921t12.824663-7.8921l41.433526-83.853565q1.973025-2.959538 1.973025-6.905587V617.55684h238.736031q16.770713 0 29.102119-12.331406T848.400771 575.136802q0-64.123314-43.406551-118.381503-40.447013-50.312139-107.529865-80.894027z" p-id="1769" fill="white"></path></svg>' : '<svg t="1759978010518" class="icon" viewBox="0 0 1024 1024" version="1.1" xmlns="http://www.w3.org/2000/svg" p-id="1559" width="200" height="200"><path d="M741 373.2l-11.4-85.2H784c26.4 0 48-21.6 48-48V48c0-26.4-21.6-48-48-48H240C213.6 0 192 21.6 192 48v192c0 26.4 21.6 48 48 48h54.4l-11.4 85.2C187.2 438.8 128 541.4 128 656c0 26.4 21.6 48 48 48h288v208c0 1.8 0.2 3.4 0.8 5l32 96c4.8 14.6 25.6 14.6 30.4 0l32-96c0.6-1.6 0.8-3.4 0.8-5V704h288c26.4 0 48-21.6 48-48 0-114.6-59.2-217.2-155-282.8zM229 608c16.6-77 71.2-140 143-175.6L404 192H288V96h448v96h-116l32 240.4c71.6 35.6 126.4 98.8 143 175.6z" p-id="1560" fill="white"></path></svg>';
        saveImages();
    });
...
function enterEditingMode(imgData) {
  if (isEditing && editingImg && currentImg && currentImg.id === imgData.id) return;
  ...
  const wrap = shadowRoot.querySelector(`.annotation-image[data-id="${imgData.id}"]`);
  if (!wrap) return;
...
function exitEditingMode() {
  ...
  isEditing = false;
  if (currentImg) {
    const wrap = shadowRoot.querySelector(`.annotation-image[data-id="${currentImg.id}"]`);
    if (wrap) {
...
function onMouseDown(e) {
  ...
  if (isEditing && currentImg) {
    ...
    if (targetWrap && targetWrap.dataset.id === currentImg.id && !isControlBtn) {
      startMoving(x, y);
      e.preventDefault();
      e.stopPropagation();
      return;
...
function onMouseMove(e) {
  ...
  if (isResizing) {
    ...
    if (handle) {
      drawingContainer.style.cursor = getCursorForHandle({ type: handle.dataset.type });
    } else if (pointInRect(x, y, currentImg.x, currentImg.y, currentImg.width, currentImg.height)) {
...
function createHandles(wrap, img) {
  const handles = [
    { type: 'nw', x: -5, y: -5 }, { type: 'n', x: img.width / 2 - 5, y: -5 }, { type: 'ne', x: img.width - 5, y: -5 },
    { type: 'e', x: img.width - 5, y: img.height / 2 - 5 }, { type: 'se', x: img.width - 5, y: img.height - 5 },
    { type: 's', x: img.width / 2 - 5, y: img.height - 5 }, { type: 'sw', x: -5, y: img.height - 5 }, { type: 'w', x: -5, y: img.height / 2 - 5 }
  ];
  handles.forEach(h => {
...
function startResizing(handle, x, y) {
  ...
  moveStartX = x;
  moveStartY = y;
  originalRect = { x: currentImg.x, y: currentImg.y, width: currentImg.width, height: currentImg.height };
...
async function saveImages() {
  try {
    ...
    const imagesStore = result.images;
    imagesStore[pageKey] = images;
    await chrome.storage.local.set({ images: imagesStore });
  } catch (error) {
    console.error(error);
  }
}
