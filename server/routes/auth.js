...
router.get("/verify", async (req, res) => {
  try {
    ...
    res.status(401).json({ status: "error", message: "Token无效或者已过期" });
  }
});
...
router.post("/register", async (req, res) => {
  try {
    ...
    if (existingUser) {
      return res.status(400).json({ status: "error", message: "用户名或邮箱已存在" });
    }
    ...
    res.status(500).json({ status: "error", message: "注册失败：" + error.message });
  }
});
...
router.post("/login", async (req, res) => {
  try {
    ...
    res.status(500).json({ status: "error", message: "登录失败：" + error.message });
  }
});
