...
### `throw new Error(...)` 和 `console.error(...)` 对比

------

| 特性                     | `throw new Error(...)`              | `console.error(...)`                   |
| ------------------------ | ----------------------------------- | -------------------------------------- |
| **目的**                 | 中断程序流程，**抛出异常**          | 仅打印错误信息到控制台，**不中断**程序 |
| **是否中断后续代码执行** | ✅ 会中断，除非被 `try...catch` 捕获 | ❌ 不会中断，后续代码继续执行           |
| **是否能被捕获**         | ✅ 可以被 `try...catch` 捕获         | ❌ 不能被捕获，只是日志                 |
| **返回值/类型**          | 抛出一个 **Error 对象**（或任意值） | 返回 `undefined`，仅打印字符串         |
| **使用场景**             | 表示**程序出错**，需要上层处理      | 表示**调试信息**，用于开发阶段排查问题 |
| **对性能的影响**         | 抛出异常会中断调用栈，性能开销大    | 几乎无性能影响                         |
...
返回的 img 值为 undefined，若控制台打印变量会先打印空值 img，后打印有值 response.image：
...
async function grabViewport() {
    ...
    return img; // 立即返回，img 还是 undefined
}
```
...
把“等两帧 + 截图”整体包进一个 **`Promise`**，用 `await` 把结果拿回来：
...
| 特性                         | `border`                              | `outline`                                          |
| ---------------------------- | ------------------------------------- | -------------------------------------------------- |
| **是否占据空间**             | ✅ 算在盒模型内，**会撑大元素尺寸**    | ❌ **不占任何空间**，像“投影”一样浮在外面           |
| **是否影响布局**             | ✅ 会触发重排（reflow）                | ❌ 只触发重绘（repaint），性能更好                  |
| **能否分别设置四边**         | ✅ 可以 `border-top/right/bottom/left` | ❌ 只能整体设置，**没有** `outline-top` 等          |
| **能否圆角**                 | ✅ 支持 `border-radius`                | ❌ **永远直角**，无法圆角                           |
| **能否设置偏移**             | ❌ 紧贴元素                            | ✅ `outline-offset: ±Npx` 可向外/内偏移             |
| **堆叠顺序**                 | 在 margin 之内，background 之上       | 在 border 之外，**覆盖在元素最上层**（z 顺序更高） |
| **是否参与盒模型计算**       | ✅ `width/height` 包含 `border-box`    | ❌ 完全忽略                                         |
| **是否跟随 `border-radius`** | ✅                                     | ❌ 永远矩形                                         |
| **浏览器默认焦点样式**       | ❌ 无                                  | ✅ `a:focus{outline:1px dotted}` 就是浏览器默认蓝框 |
...
WebSocket协议状态码是一个16位的整数，用于表示WebSocket连接的状态。状态码的第一个数字表示状态的分类，后三个数字表示具体的状态。

根据WebSocket协议的规范，状态码有以下**分类**：
...
1. **增加请求体大小限制**  
   调整服务器（如 Express）默认的 JSON 或 URL 编码请求体大小限制，从默认的 100KB 提升至数十 MB，临时解决大请求被拦截的问题。

2. **添加错误处理机制**  
   在同步逻辑中加入对 413 状态码的识别与处理，避免前端无感知失败，并可触发降级策略（如压缩、跳过同步等）。
...
3. **Canvas 数据压缩**  
   将 Canvas 导出的 PNG 格式改为 JPEG，并设置压缩质量（如 0.7），显著减小 base64 数据体积。可额外检测 Canvas 是否为空，避免无意义存储。

4. **图片数据压缩**  
   对用户上传的图片进行压缩处理：  
   - 限制最大宽高  
   - 降低 JPEG 质量  
   - 文件超过一定大小才压缩，避免过度处理小图

5. **仅同步变化部分（差量同步）**  
   不每次都同步完整数据，只同步发生变动的字段或元素，减少单次请求体积。
...
6. **分离大文件与元数据**  
   图片/Canvas 等大文件不再随标注数据一起同步，而是：  
   - 上传至文件服务器或对象存储  
   - 只同步文件 URL 或唯一标识符  
   实现“重数据走文件，轻数据走接口”的分离策略。

7. **分批同步（分片上传）**  
   若数据总量仍大，将标注数据按类型或数组长度拆分为多个小块，分多次请求上传，避免单次请求过载。
...
8. **同步前体积检查与预警**  
   在发送请求前计算数据大小，若超过阈值（如 5MB），则跳过同步或提示用户，防止频繁触发 413。

9. **延迟与重试机制**  
   对同步失败（如 413）进行延迟重试或降级处理，避免因一次性失败导致数据丢失或用户操作中断。
...
### `.populate()` 和 `ref` 跨集合关联查询

1. 存数据时：Mongoose 只把 **`_id`** 作为普通字段写入数据库，**不存任何嵌套**。  
2. 查数据时：`.populate()` 先拿这些 `_id`，再**额外发一次（或批量）查询**去目标集合把文档取回来，最后拼成对象给你。

---

**一、ref：先打标记**

`ref` 是 Schema 字段的一个声明式配置，告诉 Mongoose 这个字段存的是另一个集合的 `_id`，以后我要通过它做关联。”

```js
// User 模型
const userSchema = new mongoose.Schema({
  name: String,
  posts: [{ type: mongoose.Schema.Types.ObjectId, ref: 'Post' }] // 关键：ref
});
...
- `ref` 的值必须等于目标模型的名字（`mongoose.model('xxx')` 的第一个参数）。  
- 字段类型必须是 `ObjectId`（或其数组），否则后续无法 `.populate()`。

---

**二、.populate()：再做连接**

`.populate()` 是查询实例的方法，用来把刚才存的“外键”替换成真正的子文档，相当于 SQL 里的 JOIN。

```js
// 把用户下的所有帖子从“id 数组”变成“文档数组”
const user = await User.findOne({ name: 'Tom' }).populate('posts');
console.log(user.posts[0].title); // 直接访问帖子属性
```

| 场景                | 示例                                                         |
| ------------------- | ------------------------------------------------------------ |
| 只填充指定字段      | `.populate('posts', 'title createdAt')`                      |
| 多级填充            | `.populate({ path: 'posts', populate: { path: 'comments' } })` |
| 条件过滤            | `.populate({ path: 'posts', match: { status: 'published' } })` |
| 指定要/不要哪些字段 | `.populate({ path: 'posts', select: 'title -_id' })`         |
...
------------------------------------------------
一、**核心方法**  
| 场景     | 原生 Mongo Shell               | Node 驱动                    | Mongoose 链式写法              |
| -------- | ------------------------------ | ---------------------------- | ------------------------------ |
| 查       | `db.x.find(filter,projection)` | `col.find(filter,opts)`      | `Model.find(filter,fields)`    |
| 排序     | `.sort({age:-1})`              | `.sort({age:-1})`            | `.sort('-age')`                |
| 分页     | `.skip(20).limit(10)`          | `.skip(20).limit(10)`        | `.skip(20).limit(10)`          |
| 计数     | `db.x.countDocuments(filter)`  | `col.countDocuments(filter)` | `Model.countDocuments(filter)` |
| 去重     | `db.x.distinct('city')`        | `col.distinct('city')`       | `Model.distinct('city')`       |
| 聚合     | `db.x.aggregate([...])`        | `col.aggregate([...])`       | `Model.aggregate([...])`       |
| 联表填充 | 手动 `$lookup` + 处理          | 手动 `$lookup`               | `.populate('field','select')`  |
...
| 类别 | 操作符                                   | 记忆秒例                          | 说明                 |
| ---- | ---------------------------------------- | --------------------------------- | -------------------- |
| 比较 | `$eq` `$ne` `$gt` `$gte` `$lt` `$lte`    | `{age:{$gte:18}}`                 | 等于/不等于/大小于   |
| 包含 | `$in` `$nin`                             | `{status:{$in:["A","B"]}}`        | 在/不在数组内        |
| 存在 | `$exists` `$type`                        | `{avatar:{$exists:false}}`        | 有无字段、类型       |
| 正则 | `$regex` 或 `/pattern/`                  | `{name:/^li/i}`                   | 模糊匹配             |
| 逻辑 | `$and` `$or` `$not` `$nor`               | `{$or:[{a:1},{b:2}]}`             | 多条件组合           |
| 数组 | `$all` `$elemMatch` `$size`              | `{tags:{$all:["node","mongo"]}}`  | 全包含、子条件、长度 |
| 嵌套 | 点路径                                   | `{"addr.city":"sz"}`              | 子字段任意层         |
| 投影 | `1/0` `$` `$slice`                       | `db.x.find({},{name:1,_id:0})`    | 返回字段控制         |
| 分页 | `.sort()` `.skip()` `.limit()`           | `.sort({createdAt:-1}).limit(20)` | 排序+截取            |
| 聚合 | `$match` `$group` `$lookup` `$project` … | 见下例                            | 多阶段管道           |
...
1. 字段字符串投影  
```js
User.find({age:{$gte:18}}, 'name email -_id')   // 1 保留 0 剔除
```
...
Room
  .find()
  .populate('owner', 'username email')      // 把 owner 从 _id 换成用户子文档
  .populate({ path:'members', select:'username' })
```
底层仍是多次查询，但对开发者相当于 SQL 的 JOIN。
...
Model.aggregate([
  ...
  { $unwind: "$items" },                          // 阶段2 拆数组
  { $group: { _id: "$items.sku", total: { $sum: "$items.qty" } } }, // 阶段3 分组
  { $sort: { total: -1 } },                       // 阶段4 排序
  { $limit: 10 }                                  // 阶段5 取 Top
])
```
